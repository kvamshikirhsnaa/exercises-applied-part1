...
class Module02 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  test ("List immutability") {
    ...
    val oneTwoThreeFour = (oneTwo._1,oneTwo._2,threeFour._1,threeFour._2)
    
    oneTwo should be (1,2)
    threeFour should be (3,4)
    oneTwoThreeFour should be (1,2,3,4)
  }

  test ("List cons") {
    val twoThree = List(2,3)
    val newList = (1,twoThree(0),twoThree(1))
    val newList2 = (1,twoThree(0),twoThree(1))

    newList should be (1,2,3)
    newList2 should be (1,2,3)
  }
