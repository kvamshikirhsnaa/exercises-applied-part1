...
class Module06 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  test ("Detect any odds/evens in a list") {
    ...
    def containsOdd(nums: List[Int]): Boolean = nums.exists(x => x % 2 != 0)
  ...
  test ("With file contents control structure") {
    ...
    def withFileContents(file: File)(fn: String => String): String = {
    ...
    val fileLoc1 = new File(projectDir,"quote.txt" )
    val fileLoc2 = new File(projectDir,"sum.txt" )

     val palindrome = withFileContents(fileLoc1) { str => str.reverse }
    palindrome should be ("Madam, I'm Adam")

    val total = withFileContents(fileLoc2) { str =>
